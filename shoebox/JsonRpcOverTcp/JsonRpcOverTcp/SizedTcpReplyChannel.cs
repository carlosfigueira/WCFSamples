...
namespace JsonRpcOverTcp
{
    class SizedTcpReplyChannel : SizedTcpBaseChannel, IReplyChannel
    {
        ...
        {
            Message request = this.Receive(timeout);
        ...
        class ReceiveRequestAsyncResult : AsyncResult
        {
                ...
                : base(callback, state)
            {
                ...
                {
                    IAsyncResult receiveMessageResult = channel.BeginReceive(OnReceiveMessage, this);
            ...
            void CompleteReceiveMessage(IAsyncResult result)
            {
                try
                {
                    Message message = this.channel.EndReceive(result);
        ...
        {
            SizedTcpReplyChannel channel;
            static AsyncCallback receiveCallback = new AsyncCallback(OnReceiveRequest);
            bool receiveSuccess;
            RequestContext requestContext;
                ...
                : base(callback, state)
            {
                ...
                if (!channel.IsDisposed)
                {
                    try
                    {
                        IAsyncResult beginReceiveRequestResult = this.channel.BeginReceiveRequest(timeout, receiveCallback, this);
